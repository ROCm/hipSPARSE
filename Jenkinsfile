...
properties([
    ...
    // parameters([booleanParam( name: 'push_image_to_docker_hub', defaultValue: false, description: 'Push hipsparse image to rocm docker-hub' )]),
...
{
  String build_image_name = "build-hipsparse-hip-artifactory"
      ...
      timeout(time: 2, unit: 'HOURS')
      {
        if(isJobStartedByTimer())
        {
          sh """#!/usr/bin/env bash
                ...
                LD_LIBRARY_PATH=/opt/rocm/hcc/lib ./hipsparse-test${build_type_postfix} --gtest_output=xml --gtest_color=yes #--gtest_filter=*nightly*
            """
        ...
        else
        {
          sh """#!/usr/bin/env bash
                set -x
                cd ${paths.project_build_prefix}/build/release/clients/tests
                LD_LIBRARY_PATH=/opt/rocm/hcc/lib ./hipsparse-test${build_type_postfix} --gtest_output=xml --gtest_color=yes #--gtest_filter=*checkin*
            """
          junit "${paths.project_build_prefix}/build/release/clients/tests/*.xml"
        }
      }
...
// String docker_test_install( String hcc_ver, String artifactory_org, String from_image, String hipsparse_src_rel, String build_dir_rel )
String docker_test_install( compiler_data compiler_args, docker_data docker_args, project_paths hipsparse_paths, String job_name )
{
  def hipsparse_install_image = null
  String image_name = "hipsparse-hip-${compiler_args.compiler_name}-ubuntu-16.04"
  String docker_context = "${compiler_args.build_config}/${compiler_args.compiler_name}"
  ...
  stage( "Install ${compiler_args.compiler_name} ${compiler_args.build_config}" )
  {
    ...
    sh  """#!/usr/bin/env bash
        ...
        cp -r ${hipsparse_paths.project_src_prefix}/docker/* ${docker_context}
      """

    // Docker 17.05 introduced the ability to use ARG values in FROM statements
    // Docker inspect failing on FROM statements with ARG https://issues.jenkins-ci.org/browse/JENKINS-44836
    // hipsparse_install_image = docker.build( "${job_name}/${image_name}:${env.BUILD_NUMBER}", "--pull -f ${build_dir_rel}/dockerfile-hipsparse-ubuntu-16.04 --build-arg base_image=${from_image} ${build_dir_rel}" )
    ...
    hipsparse_install_image = docker.image( "${job_name}/${image_name}" )
  }
...
String hip_integration_testing( String inside_args, String job, String build_config )
{
  ...
  def hipsparse_integration_image = null
  ...
  String hipsparse_src_rel = checkout_and_version( job )

  // Conctruct a binary directory path based on build config
  String hipsparse_bin_rel = build_directory_rel( build_config )

  // Build hipsparse inside of the build environment
  hipsparse_integration_image = docker_build_image( job, testing_org_name, '', hipsparse_src_rel, "${testing_org_name}/${hip_test_image_name}" )

  docker_build_inside_image( hipsparse_integration_image, inside_args, job, '', build_config, hipsparse_src_rel, hipsparse_bin_rel )
...
def build_pipeline( compiler_data compiler_args, docker_data docker_args, project_paths hipsparse_paths, def docker_inside_closure )
{
  ansiColor( 'vga' )
  {
    ...
    {
      // Checkout source code, dependencies and version files
      checkout_and_version( hipsparse_paths )

      // Conctruct a binary directory path based on build config
      build_directory_rel( hipsparse_paths, compiler_args );

      // Create/reuse a docker image that represents the hipsparse build environment
      def hipsparse_build_image = docker_build_image( docker_args, hipsparse_paths )

      // Print system information for the log
      hipsparse_build_image.inside( docker_args.docker_run_args, docker_inside_closure )
    ...
    if( !hipsparse_paths.project_name.equalsIgnoreCase( 'hipsparse-hcc-ctu' ) )
    {
      ...
      String hipsparse_image_name = docker_test_install( compiler_args, docker_args, hipsparse_paths, job_name )

      docker_clean_images( job_name, hipsparse_image_name )
    }
  }
}
