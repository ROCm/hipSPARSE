...
template <typename T>
void testing_csrmv2_bad_arg(void)
{
    int n                       = 100;
    int m                       = 100;
    int nnz                     = 100;
    ...
    std::unique_ptr<csrmv2_struct> unique_ptr_csrmv2(new csrmv2_struct);
    csrmv2Info_t info = unique_ptr_csrmv2->info;
    ...
    // testing hipsparseXcsrmv2_analysis

    // testing for(nullptr == dptr)
    {
        int* dptr_null = nullptr;

        status = hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr, dptr_null, dcol, info);
    ...
    // testing for(nullptr == dcol)
    {
        ...
        status = hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr, dptr, dcol_null, info);
    ...
    // testing for(nullptr == descr)
    {
        ...
        status = hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr_null, dptr, dcol, info);
    ...
    // testing for(nullptr == info)
    {
        ...
        status = hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr, dptr, dcol, info_null);
    ...
    // testing for(nullptr == handle)
    {
        ...
        status = hipsparseXcsrmv2_analysis(handle_null, transA, m, n, nnz, descr, dptr, dcol, info);
        verify_hipsparse_status_invalid_handle(status);
    }

    // testing hipsparseXcsrmv2
    ...
    // testing for(nullptr == dptr)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr, dval, dptr_null, dcol, info, dx, &beta, dy);
        verify_hipsparse_status_invalid_pointer(status, "Error: dptr is nullptr");
    }
    // testing for(nullptr == dcol)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr, dval, dptr, dcol_null, info, dx, &beta, dy);
        verify_hipsparse_status_invalid_pointer(status, "Error: dcol is nullptr");
    }
    // testing for(nullptr == dval)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr, dval_null, dptr, dcol, info, dx, &beta, dy);
    ...
    // testing for(nullptr == dx)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr, dval, dptr, dcol, info, dx_null, &beta, dy);
    ...
    // testing for(nullptr == dy)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr, dval, dptr, dcol, info, dx, &beta, dy_null);
    ...
    // testing for(nullptr == d_alpha)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, d_alpha_null, descr, dval, dptr, dcol, info, dx, &beta, dy);
    ...
    // testing for(nullptr == descr)
    {
        ...
        status = hipsparseXcsrmv2(
            handle, transA, m, n, nnz, &alpha, descr_null, dval, dptr, dcol, info, dx, &beta, dy);
        verify_hipsparse_status_invalid_pointer(status, "Error: descr is nullptr");
    }
    // testing for(nullptr == handle)
    {
        ...
        status = hipsparseXcsrmv2(
            handle_null, transA, m, n, nnz, &alpha, descr, dval, dptr, dcol, info, dx, &beta, dy);
        verify_hipsparse_status_invalid_handle(status);
    }
}
...
template <typename T>
hipsparseStatus_t testing_csrmv2(Arguments argus)
{
    int safe_size                 = 100;
    int m                         = argus.M;
    int n                         = argus.N;
    ...
    if(m <= 0 || n <= 0 || nnz <= 0)
    {
        ...
        // Analysis
        status = hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr, dptr, dcol, info);
    ...
    // csrmv2 analysis
    CHECK_HIPSPARSE_ERROR(hipsparseXcsrmv2_analysis(handle, transA, m, n, nnz, descr, dptr, dcol, info));
    ...
    if(argus.unit_check)
    {
        ...
        CHECK_HIPSPARSE_ERROR(hipsparseSetPointerMode(handle, HIPSPARSE_POINTER_MODE_HOST));
        CHECK_HIPSPARSE_ERROR(hipsparseXcsrmv2(
        ...
        // CPU - do the csrmv2 row reduction in the same order as the GPU
        ...
        for(int i = 0; i < m; ++i)
        {
            ...
            for(int j = hcsr_row_ptr[i] - idx_base;
